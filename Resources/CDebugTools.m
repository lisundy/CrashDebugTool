...
@implementation CDebugTools
{
    NSUncaughtExceptionHandler *previousExceptionHandler;
}

void UncaughtExceptionHandler(NSException *exception) {
    [[CDebugTools sharedInstance] dealWithException:exception];
}


+ (instancetype)sharedInstance
{
    static CDebugTools *_instance;
...
- (void)showExceptionTools{
    if (@available(iOS 8.0, *)) {
        ...
        UIAlertAction *crashAction = [UIAlertAction actionWithTitle:@"crash 日志" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            [CDebugInfoListViewController presentWithType:CDebugTypeCrashInfo];
        }];
...
- (void)recordException:(NSException *)exception{
    ...
    CExceptionInfo *info = [CExceptionInfo new];
    info.date = dateStr;
...
- (NSArray *)crashInfoList {
    ...
    for (NSString *file in files) {
        ...
        CExceptionInfo *info = [NSKeyedUnarchiver unarchiveObjectWithData:data];
